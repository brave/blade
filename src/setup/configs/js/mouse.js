...
Mouse.prototype = {
    ...
    _handleMouseButton: function (e, down) {
		...
		// [MV] only log/report on mouse press, not on release 
		if (down == 1){
			var data = {"action": "touch", "X":pos.x, "Y":pos.y};
            sendPost(data)
		}
        stopEvent(e);
    },
    ...
    _generateWheelStepY: function () {
		// report data when wheel step is big enough. Missing event associated with a timeout though
		//var data = {"action": "wheel", "deltaX": dX, "deltaY": dY, "X": this._pos.x, "Y": this._pos.y};
		var data = {"action": "wheel", "deltaY": this._accumulatedWheelDeltaY, "X": this._pos.x, "Y": this._pos.y};
		sendPost(data)   
	...
	*/
	 _generateWheelStepY: function () {
		// report data when wheel step is big enough. Missing event associated with a timeout though
		//var data = {"action": "wheel", "deltaX": dX, "deltaY": dY, "X": this._pos.x, "Y": this._pos.y};
		//console.log("scroll detected and ignored")
        this._accumulatedWheelDeltaY = 0;
    },
